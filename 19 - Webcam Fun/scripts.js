...
function paintToCanvas(){
    ...
    return setInterval(() => {
        ...
        //pega os pixeis da imagem 
        let pixels = ctx.getImageData(0, 0, width, height);
        // mexe com a cor deles
        // pixels = redEffect(pixels);

        // pixels = rgbSplit(pixels);
        // ctx.globalAplha = 0.5;

        pixels = greenSceen(pixels);
...
function takePhoto(){
    ...
    // pegando os dados do canvas
    const data = canvas.toDataURL('image/jpeg');
    const link = document.createElement('a');
    link.href = data;
    link.setAttribute('download', 'BUNITU');
    link.innerHTML = `<img src="${data}" alt="Homi bunitu" />` 
    strip.insertBefore(link, strip.firstChild)
}

function redEffect(pixels){
   for(let i= 0; i < pixels.data.length; i += 4){
    pixels[i + 0] = pixels.data[i + 0] + 200; // r
